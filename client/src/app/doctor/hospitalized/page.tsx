...
import { petCareColumn } from "@/app/doctor/hospitalized/_components/columnTypes";
import {
  PetCareModel,
  PetCareResponse,
} from "@/app/doctor/hospitalized/_components/type";
import UpdatePetCare from "@/app/doctor/hospitalized/updatePetCare";
...
export default function AppointmentPage() {
  ...
  const [petCareRes, setPetCareRes] = useState<PetCareResponse>();
  ...
  const handlePagination = async (page: number = pagination.current) => {
    setIsLoading(true);
    try {
      const res = await instance.get("/api/v1/petCare/doctor/all", {
        params: {
          page: page - 1,
        },
      });
      ...
      setPetCareRes(tempRes);
      setPetCares(tempRes.content);
      setPagination({
        current: page,
  ...
  const handleDischarge = async (id: string) => {
    setIsLoading(true);
    try {
      const res = await instance.put(`/api/v1/petCare/discharge/${id}`);
      if (res.data.status === 200 || res.data.status === 201) {
        openNotification("success", "deleted successfully");
        handleEvent();
      } else {
    ...
    <>
      <div className="container py-5">
          ...
          <Col>
            {hasSelected ? (
              ...
              <Space>
                <span style={{ color: "grey", lineHeight: "12px" }}>
                  Totals{" "}
                  {!isLoading ? petCareRes?.totalElements : <LoadingOutlined />}{" "}
                  records
                </span>
                <Divider type="vertical" />
        ...
        <Table
          ...
          columns={petCareColumn}
          dataSource={petCares.map((petCare: any) => ({
            ...petCare,
            onUpdate: () => handleUpdate(petCare.id),
            onRemove: () => handleDelete(petCare.id),
            onDischarge: () => handleDischarge(petCare.id),
