...
export default function UpdateAppointment(props: AppointmentModal) {
  ...
  const fetchDataAppointment = async () => {
    ...
    try {
      ...
      if (res.data.status === 200 || res.data.status === 201) {
        setDoctorOptions([
          {
            value: res.data.data.doctorId,
            label: res.data.data.doctorName,
          },
        ]);
        // handleChangeDatePicker(dayjs(res.data.data.appointmentDate, "MORNING"));
        form.setFieldsValue({
          ...res.data.data,
